return {
  ...
  config = function()
    ...
    local fold_patterns = {
      ...
      rust = { '^use ', '^#!%[', '^mod ' }, -- Only fold #![...] (crate-level attributes), not #[...]
    }
    ...
    local function auto_fold_template(should_jump)
      ...
      -- Check if there's already a fold at line 1
      if vim.fn.foldclosed(1) ~= -1 then
        -- Fold already exists, don't create another one
        return
      end

      if ft == 'cpp' then
        -- Only fold CONSECUTIVE lines from the top
        for i, line in ipairs(lines) do
          -- Skip empty lines at the start
          if line:match '^%s*$' then
            -- Empty line, continue
          elseif line:match '^#include' or line:match '^#define' or line:match '^using' then
            last_line = i
          else
            -- Hit a non-matching line, stop looking
            break
          end
        end
      elseif ft == 'python' then
        for i, line in ipairs(lines) do
          if line:match '^%s*$' then
            -- Empty line, continue
          elseif line:match '^import%s' or line:match '^from%s+[%w_.]+%s+import' then
            last_line = i
          else
            break
          end
      ...
      elseif ft == 'rust' then
        for i, line in ipairs(lines) do
          if line:match '^%s*$' then
            -- Empty line, continue
          elseif line:match '^use%s' or line:match '^#!%[' or line:match '^mod%s' then
            last_line = i
          else
            -- Stop at first non-matching line (like 'pub fn' or anything else)
            break
          end
        end
      else
        ...
        if current_file and current_file ~= '' then
          ...
          if ext then
            ...
            if current_file:sub(1, #cwd) == cwd then
              ...
              -- Auto-regenerate Cargo.toml for Rust files
              if ext == 'rs' then
                local update_script = cwd .. '/update_cargo.sh'
                if vim.fn.filereadable(update_script) == 1 then
                  vim.fn.jobstart({ 'bash', update_script }, { detach = true })
                end
              end
            end
          end
